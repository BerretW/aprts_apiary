...
AddEventHandler("bees:addHive", function(apiaryId)
    ...
    if VORP_INV:getItemCount(_source, nil, Config.Items.HiveSmall) > 0 then
        if VORP_INV:subItem(_source, Config.Items.HiveSmall, 1) then
            ...
            TriggerClientEvent("vorp:NotifyLeft", _source, "Včelařství", "Přidán úl #" .. newId,
...
local function ProcessHiveLogic(hive, envData, apiaryId)
    if Config.Debug then
        print(json.encode(hive, {
            indent = true
        }))
        print(json.encode(envData, {
            indent = true
        }))
    end
    if not hive.hasQueen then
        debugPrint("Úl " .. hive.inventoryId .. " nemá královnu, přeskočeno.")
        return
    end

    -- 1. STÁRNUTÍ
    if not hive.queenGenetics.lifespan then
        hive.queenGenetics.lifespan = Config.QueenLifespan
    end
...
AddEventHandler("bees:applySmoker", function(apiaryId, hiveId)
    ...
    if apiary and apiary.hives[hiveId] then
        ...
        TriggerClientEvent("vorp:NotifyLeft", source, "Včelařství", "Včely uklidněny.", "generic_textures", "tick",
            4000)
    end
...
AddEventHandler("bees:applyMedicine", function(apiaryId, hiveId)
    ...
    if exports.vorp_inventory:subItem(_source, Config.Items.Medicine, 1) then
        ...
        if apiary and apiary.hives[hiveId] then
            ...
            TriggerClientEvent("vorp:NotifyLeft", _source, "Včelařství", "Vyléčeno.", "generic_textures", "tick",
                4000)
            SaveApiaryToDB(apiaryId, false)
        end
    else
...
AddEventHandler("bees:insertQueen", function(apiaryId, hiveId, item)
    ...
    if exports.vorp_inventory:subItem(_source, Config.Items.Queen, 1, item.metadata) then
        ...
        hive.stats.health = 100
        TriggerClientEvent("vorp:NotifyLeft", _source, "Včelařství", "Královna vložena.", "generic_textures",
            "tick", 4000)
        SaveApiaryToDB(apiaryId, false)
    end
    RefreshClientMenu(_source, apiaryId)
...
AddEventHandler("bees:processFrames", function()
    ...
    if VORP_INV:subItem(_source, Config.Items.FrameFull, 1) then
        ...
        TriggerClientEvent("vorp:NotifyLeft", _source, "Medomet", "Nemáš plné rámky.", "menu_textures", "cross",
            4000)
    end
end)
...
AddEventHandler("bees:insertFrame", function(apiaryId, hiveId)
    ...
    if not apiary then
        print("ERROR: Včelín " .. tostring(apiaryId) .. " nenalezen.")
        return
    end
    -- print(json.encode(apiary.hives, { indent = true }))
    -- Konverze na číslo (pro jistotu)
    local hId = tonumber(hiveId)
    local hive = apiary.hives[hId]

    -- TOTO ZDE CHYBĚLO: Kontrola, zda úl skutečně existuje
    if not hive then
        TriggerClientEvent("vorp:NotifyLeft", _source, "Chyba", "Úl #" .. tostring(hiveId) .. " nenalezen!",
            "menu_textures", "cross", 4000)
        return
    end

    -- Teď už je bezpečné volat validaci
    ...
    if VORP_INV:subItem(_source, Config.Items.FrameEmpty, 1) then
        ...
        TriggerClientEvent("vorp:NotifyLeft", _source, "Včelařství", "Vložen rámek.", "generic_textures", "tick",
            3000)
        SaveApiaryToDB(apiaryId, false)

        -- Zde voláš tu funkci pro refresh menu (podle toho, jak sis ji pojmenoval v předchozím kroku)
        -- Buď RefreshClientMenu(_source, apiaryId) nebo ten TriggerEvent logic
        RefreshClientMenu(_source, apiaryId)
        TriggerEvent("bees:requestMenuData", apiaryId)
    else
        TriggerClientEvent("vorp:NotifyLeft", _source, "Chyba", "Nemáš prázdný rámek.", "menu_textures", "cross",
            4000)
    end
end)
...
AddEventHandler("bees:harvestFrame", function(apiaryId, hiveId)
    ...
    if hive.production.filledFrames > 0 then
        ...
        TriggerClientEvent("vorp:NotifyLeft", _source, "Včelařství", "Rámek vyjmut.", "generic_textures", "tick",
            4000)
        SaveApiaryToDB(apiaryId, false)
        RefreshClientMenu(_source, apiaryId)
    else
